...
Put UTF-8 `.txt` training corpora in `data/`. The folder is included, but training data and trained model checkpoints are not.
...
```bash
python train.py data/corpus.txt --steps 3000 --out model.pt
```
...
```bash
tiny-llm train data/corpus.txt --steps 3000 --out model.pt
